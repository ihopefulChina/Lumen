...
/// from the Ossuno GUI app's keychain entries (which are ACL-bound to the
...
enum ProfileStore {
    static let service = "studio.ossuno.mcp"
    ...
    private static var activePointerURL: URL {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("OssunoMCP", isDirectory: true)
...
enum ProfileStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .notFound(let name):
            return "找不到配置档案「\(name)」。先运行 ossuno-mcp auth 添加。"
