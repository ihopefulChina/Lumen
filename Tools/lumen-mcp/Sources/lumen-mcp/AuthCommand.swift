...
enum AuthCommand {
    static func run(arguments: [String]) async -> Int32 {
        ...
        case "--remove":
            guard arguments.count >= 2 else {
                FileHandle.standardError.write("用法：lumen-mcp auth --remove <profile>\n".data(using: .utf8)!)
                return 64
            }
            return remove(name: arguments[1])
        case "--use":
            guard arguments.count >= 2 else {
                FileHandle.standardError.write("用法：lumen-mcp auth --use <profile>\n".data(using: .utf8)!)
                return 64
            }
    ...
    static func printHelp() {
        print("""
        lumen-mcp auth — 管理 OSS 凭证（独立于 Lumen App 的钥匙串）

        用法：
          lumen-mcp auth                 交互式添加/更新配置档案
          lumen-mcp auth --list          列出所有配置档案
    ...
    private static func interactiveAdd() async -> Int32 {
        print("配置 Lumen MCP 的 OSS 凭证（存储在当前用户的钥匙串中）")
    ...
    private static func list() -> Int32 {
        ...
        guard !names.isEmpty else {
            print("还没有配置档案。运行 lumen-mcp auth 添加。")
            return 0
        }
    ...
    private static func use(name: String) -> Int32 {
        guard ProfileStore.listNames().contains(name) else {
            FileHandle.standardError.write("找不到配置档案「\(name)」。运行 lumen-mcp auth --list 查看。\n".data(using: .utf8)!)
            return 1
        }
        do {
