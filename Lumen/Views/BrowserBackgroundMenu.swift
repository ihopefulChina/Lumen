...
enum BrowserMenuHit: Equatable {
    ...
    static func parseIdentifier(_ raw: String?) -> BrowserMenuHit? {
        ...
        if raw.hasPrefix("lumen.folder:") {
            let value = String(raw.dropFirst("lumen.folder:".count))
            return value.isEmpty ? nil : .folder(value)
        }
        if raw.hasPrefix("lumen.file:") {
            let value = String(raw.dropFirst("lumen.file:".count))
