...
struct RealOSSSmokeTests {
    private let prefix = "ossuno-v100-smoke/"

    @Test(.enabled(
        if: realOSSSmokeEnabled,
        "Requires the explicit REAL_OSS_SMOKE compilation condition and a locally saved Ossuno account"
    ))
    func uploadListDownloadRenameAndCleanUp() async throws {
        ...
        let smallData = Data("Ossuno 1.0.0 real OSS smoke\n".utf8)
        let specialData = Data("路径与签名 smoke ✓\n".utf8)

        let localRoot = FileManager.default.temporaryDirectory
            .appending(path: "ossuno-v100-oss-smoke-\(UUID().uuidString)", directoryHint: .isDirectory)
...
private enum SmokeFailure: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noSavedAccount: "没有找到本机已保存的 Ossuno 账号"
